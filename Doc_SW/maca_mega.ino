...
void loop() {
  ...
  if (calentar == true) {   //Ciclo de lectura de sensores de gas
    analogWrite(13, 140);   //3 //Tensiones de calentamiento
    analogWrite(12, 120);   //2
    analogWrite(11, 110);   //1
...
void leer() {
  ...
  R1 = (10000.0 / (v_0 - v_2)) * v_2;      //Resistencias medidas de los sensores
  R2 = (4700.0 / (v_0 - v_4)) * v_4;      //Si se cambian los valores de resistencia de carga es necesario actualizar el '4700.0' por el valor correspondiente
  R3 = (10000.0 / (v_0 - v_8)) * v_8;      //OJO
...
void contador() {
  ...
  if (timer < 59) {
    timer++;
    if (timer == 30) {    //Aca se verifica que hayan pasado 30 segundos, que es el ciclo de lectura de sensores
      flag_lectura = true;  //flag_lectura=false;
      calentar = !calentar;   //Calentar=false
    }else if(timer>30){
      if(flag_shinyei==1){
        flag_shinyei=0;
        total_10=0;
    ...
    analogWrite(2, 0);
    flag_grabar = true;
    calentar = !calentar;     //Calentar=true;
  }
}
